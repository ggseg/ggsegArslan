...
# Make  3d ----
arslan_3d <- make_aparc_2_3datlas(
  annot = annot_name,
...
# Make palette ----
brain_pals <- make_palette_ggseg(arslan_3d)
usethis::use_data(brain_pals, internal = TRUE, overwrite = TRUE)
devtools::load_all(".")


# Make 2d polygon ----
arslan <- make_ggseg3d_2_ggseg(arslan_3d, 
                               steps = 6:7,
                               tolerance = .5,
                               output_dir = here::here("data-raw/"))
plot(arslan)
...
arslan %>%
  ...
  scale_fill_brain("arslan", package = "ggsegArslan", na.value = "black")


usethis::use_data(arslan, arslan_3d,
                  internal = FALSE,
                  overwrite = TRUE,
                  compress="xz")
